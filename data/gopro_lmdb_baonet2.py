...
class DeblurDataset(Dataset):
    def __init__(self, path, frames, future_frames, past_frames, crop_size=(256, 256), ds_type='train', centralize=True,
        ...
        # self.datapath_event = join(path, '{}_{}_warpedsbt_PpN'.format(ds_name, ds_type))
        self.datapath_event2 = join(path, '{}_{}_sbt48c'.format(ds_name, ds_type))
    ...
    def __getitem__(self, idx):
        ...
        for i in range(self.frames):
            try:
                blur_img, sharp_img, event2_stack = self.get_img(seq_idx, frame_idx + i, sample)
        ...
        # event_stacks = torch.cat(event_stacks, dim=0)#f,4,h,w
        event2_stacks = torch.cat(event2_stacks, dim=0)
        sharp_imgs = torch.cat(sharp_imgs[self.num_pf:(self.frames - self.num_ff)], dim=0)#f,4,3,h,w
        inputs = torch.cat([blur_imgs, event2_stacks], dim=1) #f,3+48,h,w
