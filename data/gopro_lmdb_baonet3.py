...
class DeblurDataset(Dataset):
    def __init__(self, path, frames, future_frames, past_frames, crop_size=(256, 256), ds_type='train', centralize=True,
                 normalize=True):
        ds_name = 'gopro'
        self.datapath_blur = join(path, '{}_{}_fullblur'.format(ds_name, ds_type))
        self.datapath_gt = join(path, '{}_{}_gt'.format(ds_name, ds_type))
        self.datapath_event = join(path, '{}_{}_optical_flow'.format(ds_name, ds_type))
        self.datapath_event2 = join(path, '{}_{}_warpedsbt_PpN'.format(ds_name, ds_type))
        print("event_path: {}".format(self.datapath_event))
        with open(join(path, '{}_info_{}_half.pkl'.format(ds_name, ds_type)), 'rb') as f:
    ...
    def __getitem__(self, idx):
        blur_imgs, sharp_imgs, event_stacks, event2_stacks = list(), list(), list(), list()
        ...
        for i in range(self.frames):
            try:
                blur_img, sharp_img, event_stack, event2_stack = self.get_img(seq_idx, frame_idx + i, sample)
                blur_imgs.append(blur_img)
                sharp_imgs.append(sharp_img)
                event_stacks.append(event_stack)
                event2_stacks.append(event2_stack)
            except TypeError as err:
                print('Handling run-time error:', err)
                print('failed case: i {}, seq_idx {}, frame_idx {}'.format(i, seq_idx, frame_idx))
        blur_imgs = torch.cat(blur_imgs, dim=0)#f,3,h,w
        event_stacks = torch.cat(event_stacks, dim=0)#f,8,h,w
        event2_stacks = torch.cat(event2_stacks, dim=0)#f,4,h,w
        sharp_imgs = torch.cat(sharp_imgs[self.num_pf:(self.frames - self.num_ff)], dim=0)#f,4,3,h,w
        inputs = torch.cat([blur_imgs, event_stacks, event2_stacks], dim=1) #f,11,h,w
#         print("inputs.shape:{}".format(inputs.shape))
        return inputs, sharp_imgs
    ...
    def get_img(self, seq_idx, frame_idx, sample):
        ...
        blur_img = blur_img.reshape(self.H, self.W, self.C).transpose((2,0,1))
        sharp_img = self.txn_gt.get(code)
        sharp_img = np.frombuffer(sharp_img, dtype='uint8')
        sharp_img = sharp_img.reshape(4, self.H, self.W, self.C).transpose((0,3,1,2))
        
        event_stack = self.txn_event.get(code)
        event_stack = np.frombuffer(event_stack, dtype='float16')
        event_stack = event_stack.reshape(8, self.H, self.W)

        event2_stack = self.txn_event2.get(code)
        event2_stack = np.frombuffer(event2_stack, dtype='float16')
        event2_stack = event2_stack.reshape(4, self.H, self.W)
        
        sample['image'] = blur_img
        sample['label'] = sharp_img
        sample['event'] = event_stack
        sample['event2'] = event2_stack
        sample['iseventfocus'] = False
        sample['isdeblurandframeinterpolation2'] = True
        sample = self.transform(sample)
        blur_img = normalize(sample['image'], centralize=self.centralize, normalize=self.normalize)
#         sharp_img = normalize(sample['label'], centralize=self.centralize, normalize=self.normalize, is5d=True)
        sharp_img = sample['label']
        event_stack = normalize(sample['event'], centralize=self.centralize, normalize=self.normalize)
        event2_stack = normalize(sample['event2'], centralize=self.centralize, normalize=self.normalize)
        #print("event_stack: {}".format(event_stack.shape))
        
        return blur_img, sharp_img, event_stack, event2_stack
