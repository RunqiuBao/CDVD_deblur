...
#         for i in range(self.frames):
        try:
            blur_img, sharp_img, event_stack = self.get_img(seq_idx, frame_idx, sample)
...
#         blur_imgs = torch.cat(blur_imgs, dim=0)#1,3,h,w
#         event_stacks = torch.cat(event_stacks, dim=0)#1,4,h,w
#         sharp_imgs = torch.cat(sharp_imgs, dim=0)#1,4,h,w,3
        inputs = torch.cat([blur_img, event_stack], dim=0)
#         print("inputs.shape:{}".format(inputs.shape))

        return inputs, sharp_img
    ...
    def get_img(self, seq_idx, frame_idx, sample):
        ...
        sharp_img = sharp_img.reshape(4, self.H, self.W, self.C).transpose((1,2,3,0))#h,w,c,4
        event_stack = self.txn_event.get(code)
        event_stack = np.frombuffer(event_stack, dtype='float16')
        event_stack = event_stack.reshape(4, self.H, self.W).transpose((1,2,0))
        ...
        b,c1,c2,h,w = sample['label'].shape
        sharp_img = sample['label'].reshape(b,12,h,w)
        sharp_img = normalize(sharp_img, centralize=self.centralize, normalize=self.normalize).reshape(b,4,3,h,w)
        ...
        return blur_img.squeeze(), sharp_img.squeeze(), event_stack.squeeze()
