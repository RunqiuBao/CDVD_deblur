...
class RDBCell(nn.Module):
    def __init__(self, para):
        ...
        self.F_B0 = conv5x5(3, 64, stride=1)
...
class Refiner(nn.Module):
    def __init__(self, para):
        ...
        # self.hd_project = conv1x1(4,8,stride=1)
        # self.f_block1 = RRDB(8, 8, reduction=1, do_attention=True)
        # self.f_concat = conv3x3(8,3)
        # self.f_gbout = conv1x1(3,3)
        # self.hd_project2 = conv1x1(6,12,stride=1)
        # self.f_block2 = RRDB(12, 8, reduction=1, do_attention=True)
        ...
        self.hd_project = conv1x1(15,6,stride=1)
        self.f_block1 = RRDB(6, 6, reduction=1, do_attention=True)
        self.f_concat = conv3x3(6,3)
        self.f_gbout = conv1x1(3,3)
        self.hd_project2 = conv1x1(6,6,stride=1)
        self.f_block2 = RRDB(6, 6, reduction=1, do_attention=True)
        self.f_dbout = conv_na(6, 6, kernel_size=3, padding=1, act_type='relu', norm_type=None)
        self.f_dbout2 = conv_na(6, 3, kernel_size=3, padding=1, act_type=None, norm_type=None)
    
    def forward(self, inputs, priors): ##inputs(n,4,3,h,w), prior:(n,48,h,w)
        ...
        G_inputs = Mgrad(inputs, self.eps, ifmog=False) #n*j,3,h,w
        for ii in range(n*j):
            ...
            dbranch = self.hd_project2(torch.cat([inputs[ii].unsqueeze(0), gbranch], dim=1))
            dbranch = self.f_block2(dbranch)
            dbout = self.f_dbout(dbranch)
            dbout = self.f_dbout2(dbout)
            dbouts.append(dbout)
        dbouts = torch.cat(dbouts, dim=0) #n*j,3,h,w
        gbouts = torch.cat(gbouts, dim=0) #n*j,3,h,w
        return gbouts.reshape(n,j,c,h,w).unsqueeze(1),  dbouts.reshape(n,j,c,h,w).unsqueeze(1)
...
class Model(nn.Module):
    ...
    def forward(self, x):
        ...
        sbt = x[:,self.num_fb:(frames - self.num_ff),3:,:,:]
        ...
        gbouts = []
        for i in range(f):
            gbout, output_refine = self.refine(outputs[:,i], sbt[:,i])
            outputs_refine.append(output_refine)
            gbouts.append(gbout)
        outputs_refine = torch.cat(outputs_refine, dim=1)#n,f-m,4,3,h,w
        outputs_refine += blurry.unsqueeze(2).repeat(1,1,4,1,1,1)
