...
class Model(nn.Module):
    ...
    def forward(self, inputs, profile_flag=False):
        ...
        tsi = inputs[:,0:2,:,:]#(4,2,h,w)
        sbt = inputs[:,2:,:,:]#(4,2,h,w)
        ...
        x = self.up3(x, x2)
        x = self.up4(x, x1)
        outputs = self.outc(x) #backward optical flow
        
        # warping the tsi to t_start with the optical flow
        outputs = outputs*tsi
        outputs = outputs.permute(0,2,3,1)
        warped_start = F.grid_sample(sbt, outputs)
        
        return outputs, warped_start #(4,h,w,2), (4,2,h,w)
