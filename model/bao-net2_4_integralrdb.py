...
# RDB module with integral
class RDB_integral(nn.Module):
    def __init__(self, in_channels, growthRate, num_layer, activation='relu'):
        super(RDB_integral, self).__init__()
    ...
    def forward(self, x, h=None):
        ...
        ix = []
        for ii in range(cc):
            ixx = torch.cumsum(x[:,ii,:,:].unsqueeze(1), dim=3)
            ixy = torch.cumsum(x[:,ii,:,:].unsqueeze(1), dim=2)
            ix.append(torch.cat([ixx,ixy], dim=1))
        ix = torch.cat(ix, dim=1)
        out = self.dense_layers(ix)
        out = self.conv1x1(out)
        out += x
        return out
...
class RDNet(nn.Module):
    def __init__(self, in_channels, growthRate, num_layer, num_blocks, activation='relu'):
        ...
        for i in range(num_blocks):
            self.RDBs.append(RDB_integral(in_channels, growthRate, num_layer, activation))
...
class RDB_DS(nn.Module):
    def __init__(self, in_channels, out_channels, growthRate, num_layer, activation='relu'):
        ...
        self.rdb = RDB_integral(in_channels, growthRate, num_layer, activation)
...
class RDBCell(nn.Module):
    def __init__(self, para):
        ...
        # self.F_IB = RDB_integral(in_channels=9, out_channels=9, growthRate=self.n_feats, num_layer=3, activation=self.activation)
        self.F_B0 = conv5x5(9, 48, stride=1)
    ...
    def forward(self, x, s_last):
        # x: n,c,h,w
        inputx = x[:,0,:,:].unsqueeze(1)
        dx = x[:,1:,:,:]
        ix = []
        for ii in range(dx.shape[1]//2):
            dxx = torch.cumsum(dx[:,(0+2*ii),:,:].unsqueeze(1), dim=3)
            dxy = torch.cumsum(dx[:,(1+2*ii),:,:].unsqueeze(1), dim=2)
            ix.append(torch.cat([dxx,dxy], dim=1))
        ix = torch.cat(ix, dim=1)
        out = torch.cat([inputx,ix], dim=1)
        out = self.F_B0(out)
        out = self.F_B1(out)
        out = self.F_B2(out)
...
class Model(nn.Module):
    ...
    def __init__(self, para):
        ...
        self.igp2 = IGP2(para)
        for param in self.igp2.parameters():
            param.requires_grad = False
        print("igp param.requires_grad1: False")
    ...
    def forward(self, x):
        ...
        sbts = x[:,:,1:,:,:]
        igps = self.igp2(sbts)#(n,10,4*6,h,w)
        # print("igps shape,", igps.shape)
        blurry = x[:,self.num_fb*2:(frames - self.num_ff*2),0,:,:].unsqueeze(2).unsqueeze(2).repeat(1,1,4,1,1,1)
        ...
        for i in range(self.num_fb, frames-self.num_ff):
            h, s = self.cell(torch.cat([x[:, i, 0,:,:].unsqueeze(1), igps[:,i-self.num_fb,:,:,:]], dim=1), s)
            hs.append(h)
        for i in range(self.num_fb*2, frames - self.num_ff*2):
            out = self.fusion(hs[i - self.num_fb*2:i - self.num_fb + self.num_ff + 1])
            out = self.recons(out)
            outputs.append(out.unsqueeze(dim=1))
        outputs = torch.cat(outputs, dim=1)
        n,f,c,h,w = outputs.shape
        outputs = outputs.reshape(n,f,4,1,h,w) #n,f-m,12,h,w
        outputs += blurry
#         print('outputs shape', outputs.shape)

        return normalize_reverse(outputs, normalize=True)#n,f-m,4,1,h,w
