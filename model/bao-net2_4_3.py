...
class RDBCell(nn.Module):
    def __init__(self, para):
        ...
        self.F_B2 = RDB_DS(in_channels=64, out_channels=32, growthRate=int(self.n_feats * 3 / 2), num_layer=3,
...
class Model(nn.Module):
    ...
    def forward(self, x):
        ...
        for i in range(self.num_fb, frames-self.num_ff):
            h, s, mid = self.cell(torch.cat([x[:, i, 0:3,:,:], igps[:,i-self.num_fb,:,:,:]], dim=1), s, mid)
            hs.append(h)
