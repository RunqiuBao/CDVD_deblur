...
from .arches import conv1x1, conv3x3, conv5x5
from .igp_ascomponents_bao import IGP2
...
class RNNCell(nn.Module):
    def __init__(self, para):
        ...
        self.F_B0 = conv5x5(27, 32, stride=1)
...
class Model(nn.Module):
    """
    IGP based video deblur and frame interpolation
    """
    def __init__(self, para):
        ...
        self.n_blocks = para.n_blocks_b # supporse n_blocks = 12
        self.skip = nn.Conv2d(in_channels=3,out_channels=12, kernel_size=9, stride=1, padding=4, bias=True)
        ...
        self.F_L = nn.Sequential(
            ...
            nn.ConvTranspose2d((5 * self.n_feats) , 36, kernel_size=3, stride=2,
    ...
    def forward(self, x):
        ...
        s_height = int(height / self.ratio)
        s_width = int(width / self.ratio)
        ...
        for i in range(self.num_fb, frames-self.num_ff):
            h, s, mid = self.rnncell0(torch.cat([x[:, i, 0:3,:,:], igps[:,i-self.num_fb,:,:,:]], dim=1), s, mid)
            hs.append(h)
        for i in range(self.num_fb*2, frames - self.num_ff*2):
            out = self.ta_fusion(hs[i - self.num_fb*2:i - self.num_fb + self.num_ff + 1])
            out = self.F_L(out)
            
            skip = self.skip(x[:, i, 0:3, :, :])
            out = out + skip
            n,c,h,w = out.shape
            out = out.reshape(n,4,3,h,w) #n,4,3,h,w
...
def feed(model, iter_samples):
    inputs = iter_samples[0]#n,f,51,h,w
