...
class RDBCell(nn.Module):
    def __init__(self, para):
        ...
        self.F_B0 = conv5x5(27, 48, stride=1)
...
def feed(model, iter_samples):
    ...
    sbts = iter_samples[2]
    outputs = model(inputs, sbts)
    return outputs


def cost_profile(model, H, W, seq_length):
    x = torch.randn(1, seq_length, 3, H, W).cuda()
    sbts = torch.randn(1, seq_length, 48, H//2, W//2).cuda()
    profile_flag = True
    flops, params = profile(model, inputs=(x,sbts), verbose=False)
