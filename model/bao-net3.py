...
class RDBCell(nn.Module):
    def __init__(self, para):
        ...
        self.F_B0 = conv5x5(7, 48, stride=1)
...
refine_feats = 12
refine_feats = int(refine_feats)

class SFTLayer(nn.Module):
    def __init__(self,para):
        super(SFTLayer, self).__init__()
        self.n_features = para.n_features
        self.SFT_scale_conv0 = nn.Conv2d(1, refine_feats, 1)
        self.SFT_scale_conv1 = nn.Conv2d(refine_feats, refine_feats, 1)
        self.SFT_shift_conv0 = nn.Conv2d(1, refine_feats, 1)
        self.SFT_shift_conv1 = nn.Conv2d(refine_feats, refine_feats, 1)

    def forward(self,x,y):
        # x[0]: fea; x[1]: cond batch_size, frames, channels, height, width = x.shape
...
#         print('outputs shape', outputs.shape)
        ...
        gbouts = []
        for i in range(f):
            gbout, output_refine = self.refine(outputs[:,i], warpedsbt.unsqueeze(3)[:,i])
...
def cost_profile(model, H, W, seq_length):
    x = torch.randn(4, seq_length, 11, H, W).cuda()
    profile_flag = True
