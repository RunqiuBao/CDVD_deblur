...
class RDBCell(nn.Module):
    def __init__(self, para):
        ...
        self.n_feats = para.n_features
        self.n_blocks = para.n_blocks_igp
        self.F_B0 = conv5x5(48, 48, stride=1)
        self.F_B1 = RDB(in_channels=48, growthRate=self.n_feats, num_layer=3, activation=self.activation)
...
class Reconstructor(nn.Module):
    def __init__(self, para):
        ...
        self.model = nn.Sequential(
            nn.ConvTranspose2d((5 * self.n_feats) * (self.related_f), 3 * self.n_feats, kernel_size=3, stride=2,
                               padding=1, output_padding=1),
            nn.ConvTranspose2d(3 * self.n_feats, 24, kernel_size=3, stride=2, padding=1, output_padding=1),
            conv5x5(24, 4*6, stride=1)
        )

    def forward(self, x):
        return self.model(x)

class IGP2(nn.Module):
    def __init__(self, para):
        super(IGP2, self).__init__()
        self.para = para
        self.n_feats = para.n_features
        self.num_ff = para.future_frames
        self.num_fb = para.past_frames
