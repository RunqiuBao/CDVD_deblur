...
class Model(nn.Module):
    ...
    def __init__(self, para):
        ...
        self.igp = IGP3(para)
        for param in self.igp.parameters():
    ...
    def forward(self, x):#(n,14,51,h,w)
        outputs, hs = [], []
        batch_size, frames, channels, height, width = x.shape
        sbts = x[:,:,3:,:,:]
        igps = self.igp(sbts)#(n,10,4*6,h,w)
        blurry = x[:,self.num_fb*2:(frames - self.num_ff*2),0:3,:,:] #(n,6)
        ...
        outputs = outputs.reshape(n,f,4,3,h,w) #n,6,4,3,h,w
