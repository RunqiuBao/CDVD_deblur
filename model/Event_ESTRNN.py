...
class RDBCell(nn.Module):
    def __init__(self, para):
        ...
        #self.hdproject1_sbt = conv1x1(44, 64)
        #self.hdproject2_sbt = conv1x1(64, 64)
        self.F_B0 = conv5x5(47, 47, stride=1)
        self.F_B1 = RDB_DS(in_channels=47, out_channels=47, growthRate=self.n_feats, num_layer=3, activation=self.activation)
        self.F_B2 = RDB_DS(in_channels=47, out_channels=64, growthRate=int(self.n_feats * 3 / 2), num_layer=3,
    ...
    def forward(self, x, s_last):
        #sbts = x[:,3:,:,:]
        #x = x[:,0:3,:,:]
        #sbts = self.hdproject1_sbt(sbts)
        #sbts = self.hdproject2_sbt(sbts)
        #x = torch.cat([x,sbts], dim=1)
        out = self.F_B0(x)
        out = self.F_B1(out)
        out = self.F_B2(out)
...
class Model(nn.Module):
    ...
    def forward(self, x, profile_flag=False):
        ...
        outputs = outputs + x[:,self.num_fb:(frames - self.num_ff),0:3,:,:]
...
def cost_profile(model, H, W, seq_length):
    x = torch.randn(1, seq_length, 47, H, W).cuda()
    profile_flag = True
