...
class RDNet(nn.Module):
    def __init__(self, in_channels, growthRate, num_layer, num_blocks, activation='relu', do_norm=False, do_attention=False):
...
class RDBCell(nn.Module):
    def __init__(self, para):
        ...
        self.F_B0 = conv5x5(27, 64, stride=1)
        ...
        self.F_R = RDNet(in_channels=(1+2+2) * self.n_feats, growthRate=2 * self.n_feats, num_layer=3,
                    num_blocks=self.n_blocks, activation=self.activation, do_norm=False,do_attention=True)  # in: 80
...
class Model(nn.Module):
    ...
    def forward(self, x):
        ...
        blurry = x[:,self.num_fb*2:(frames - self.num_ff*2),0:3,:,:]
        ...
        for i in range(self.num_fb, frames-self.num_ff):
            h, s = self.cell(torch.cat([x[:, i, 0:3,:,:], igps[:,i-self.num_fb,:,:,:]], dim=1), s)
            hs.append(h)
        ...
        outputs = outputs.reshape(n,f,4,3,h,w) #n,f-m,12,h,w
        outputs += blurry.unsqueeze(2).repeat(1,1,4,1,1,1)
#         print('outputs shape', outputs.shape)

        return normalize_reverse(outputs, normalize=True)#n,f-m,4,3,h,w
