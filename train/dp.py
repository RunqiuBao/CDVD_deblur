...
from .metrics import ssim_calc_torch
...
def train(train_loader, model, criterion, metrics, opt, epoch, para, logger):
    ...
    for iter_samples in train_loader:
        ...
        labels_list = [labels[:, i, 2, :, :, :] for i in range(para.frames)]
        labels = torch.cat([labels_list[1], labels_list[2], labels_list[3], labels_list[2]], dim=1)

        recons_1, recons_2, recons_3, recons_2_iter = model(iter_samples)
        outputs = torch.cat([recons_1, recons_2, recons_3, recons_2_iter], dim=1)
        # (n,4,h,w)

        losses = criterion(outputs, labels)
        measure = metrics(outputs.detach()[:,3].unsqueeze(1), labels[:,3].unsqueeze(1))
...
def valid(valid_loader, model, criterion, metrics, epoch, para, logger):
    ...
    with torch.no_grad():
        ...
        for iter_samples in valid_loader:
            ...
            if seq_idx in psnr_seqs.keys():
                psnr_seqs[seq_idx].update(measure.detach().item(), inputs.size(0))
            else:
                psnr_seqs[seq_idx] = AverageMeter()
                psnr_seqs[seq_idx].update(measure.detach().item(), inputs.size(0))

            a = outputs.detach().clamp(0,255.0).round()
            b = labels
            ssim = ssim_calc_torch(a, b)
            ssim_meter.update(ssim.item(), inputs.size(0))
            if seq_idx in ssim_seqs.keys():
                ssim_seqs[seq_idx].update(ssim.item(), inputs.size(0))
            else:
                ssim_seqs[seq_idx] = AverageMeter()
                ssim_seqs[seq_idx].update(ssim.item(), inputs.size(0))

            batchtime_meter.update(time.time() - end)
            end = time.time()
            pbar.update(para.batch_size)
        print('psnr, ssim in every seqs!')
        for (key, val) in enumerate(psnr_seqs):
            print(psnr_seqs[key].avg)
        for (key, val) in enumerate(ssim_seqs):
            print(ssim_seqs[key].avg)
    pbar.close()
    ...
    print('[valid] ssim: ', ssim_meter.avg)
    msg = '[valid]'
    for key, meter in losses_meter.items():
        if key == 'all':
            continue
