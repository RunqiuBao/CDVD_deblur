    ...
    if para.resume:
        if os.path.isfile(para.resume_file):
            if para.only_resume_model:
                ...
                pretrained_dict = checkpoint['state_dict']
                print('model_key:')
                model_key = [k for k,v in model_dict.items()]
                print(model_key[0])
                print('pretrained key:')
                pretrain_key = [k for k,v in pretrained_dict.items()]
                print(pretrain_key[0])
                if logger:
...
def dist_train(train_loader, model, criterion, metrics, opt, epoch, para, logger):
    ...
    for iter_samples in train_loader:
        ...
        with torch.no_grad():
            labels = labels[:,para.past_frames:(para.frames - para.future_frames)]
            n,fm,j,c,h,w = labels.shape
            labels = labels.reshape(n*fm*j,c,h,w)
            labels = Mgrad_mono(labels, para.epsilon, ifmog=False)#(n*fm*4,2,h,w)
        ...
        measure_meter.update(reduced_measure.item(), inputs.size(0)/4)
    ...
    if logger:
        ...
        logger('[train] epoch time: {:.2f}s, average batch time: {:.2f}s'.format(end - start, batchtime_meter.avg),
               timestamp=False)
        logger.report([[para.loss, 'min'], [para.metrics, 'min']], state='train', epoch=epoch)
        msg = '[train]'
...
#                 pbar.update(para.num_gpus * para.batch_size * 4)
        ...
        if logger:
            ...
            logger('[valid] epoch time: {:.2f}s, average batch time: {:.2f}s'.format(end - start, batchtime_meter.avg),
                   timestamp=False)
            logger.report([[para.loss, 'min'], [para.metrics, 'min']], state='valid', epoch=epoch)
            msg = '[valid]'
            for key, meter in losses_meter.items():
                if key == 'all':
                    continue
