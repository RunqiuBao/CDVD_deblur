...
def dist_train(train_loader, model, criterion, metrics, opt, epoch, para, logger):
    ...
    for iter_samples in train_loader:
        iter_samples = iter_samples.cuda()
        inputs = iter_samples
#         print("in loader inputs.shape: {}".format(inputs.shape))
        outputs, warped = model(iter_samples)

        # todo merge this part with model class
        losses = criterion(outputs, warped)
        for key in outputs:
            outputs[key] = outputs[key].detach()
        for key in warped:
            warped[key] = warped[key].detach()
        measure = metrics(outputs, warped)
        ...
        opt.zero_grad()
#         print("losses['all']: {}".format(losses['all'].detach()))
        losses['all'].backward()
...
def dist_valid(valid_loader, model, criterion, metrics, epoch, para, logger):
    ...
    with torch.no_grad():
        ...
        for iter_samples in valid_loader:
            ...
            losses = criterion(outputs, warped, valid_flag=True)
            for key in outputs:
                outputs[key] = outputs[key].detach()
            for key in warped:
                warped[key] = warped[key].detach()
